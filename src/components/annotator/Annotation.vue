                ...
                <div class="col-sm-8">
                  <select class="form-control" @change="setCategory">
                    <option
                      ...
                      :selected="annotation.categoryId === option.value"
                      :key="option.text"
                    >
                      {{ option.text }}
                    </option>
...
export default {
  ...
  methods: {
    ...
    initAnnotation() {
      const metaName = this.annotation.metadata.name
    ...
    createCompoundPath(json, segments) {
      ...
      const width = this.annotation.width
      const height = this.annotation.height
      ...
      const keypoints = this.annotation.keypoints
      if (keypoints) {
        for (let i = 0; i < keypoints.length; i += 3) {
          const x = keypoints[i] - width / 2
          const y = keypoints[i + 1] - height / 2
    ...
    onAnnotationKeypointSettingsClick(labelIndex) {
      this.keypoint.tag = [String(labelIndex + 1)]
      const indexLabel = parseInt(String(this.keypoint.tag))
      if (this.keypoints && indexLabel in this.keypoints._labelled) {
        const labelled = this.keypoints._labelled[indexLabel]
    ...
    createUndoAction(actionName) {
      ...
      const copy = this.compoundPath.clone()
    ...
    addKeypoint(point, visibility, label) {
      ...
      const keypoint = new Keypoint(point.x, point.y, {
        ...
        onClick: event => {
          ...
          const keypoint = event.target.keypoint
          ...
          if (this.currentKeypoint) {
            const i1 = this.currentKeypoint.indexLabel
            const i2 = keypoint.indexLabel
        ...
        onDoubleClick: event => {
          ...
          const id = `#keypointSettings${this.annotation.id}`
          const indexLabel = this.currentKeypoint.indexLabel
      ...
      const unusedLabels = this.notUsedKeypointLabels
    ...
    unite(compound, simplify = true, undoable = true, isBBox = false) {
      ...
      const newCompound = this.compoundPath.unite(compound)
    ...
    subtract(compound, simplify = true, undoable = true) {
      ...
      const newCompound = this.compoundPath.subtract(compound)
      newCompound.onDoubleClick = this.compoundPath.onDoubleClick
    ...
    export() {
      if (this.compoundPath === null) this.createCompoundPath()
      const metadata = this.$refs.metadata.export()
      ...
      const json = this.compoundPath.exportJSON({
        asString: false,
        precision: 1
      })
      ...
      this.compoundPath.fullySelected = this.isCurrent
      if (this.annotation.paperObject !== json) {
    ...
    emitModify() {
      this.uuid = Math.random()
        .toString(36)
        .replace(/[^a-z]+/g, '')
      this.annotation.paperObject = this.compoundPath.exportJSON({
        asString: false,
        precision: 1
      })
      this.$socket.emit('annotation', {
    ...
    getKeypointVisibility(index) {
      ...
      if (this.keypoints && this.keypoints._labelled) {
        const labelled = this.keypoints._labelled[index + 1]
        if (labelled) {
  ...
  computed: {
    ...
    keypointListView() {
      const listView = []
      for (let i = 0; i < this.keypointLabels.length; ++i) {
        const visibility = this.getKeypointVisibility(i)
    ...
    darkHSL() {
      ...
      const l = Math.round(color.lightness * 50)
  ...
  sockets: {
    annotation(data) {
      ...
      if (data.action === 'modify') {
        this.createCompoundPath(annotation.paperObject, annotation.segmentation)
      }
