  ...
  >
    ...
    <ul v-show="showAnnotations" ref="collapse" class="list-group">
      ...
      <Annotation
        ...
        @keypoints-compconste="$emit('keypoints-compconste')"
        :opacity="opacity"
        :index="listIndex"
...
export default {
  ...
  data: function() {
    return {
      ...
      keypoint: {
        labels: [...this.category.keypointLabels],
        edges: [...this.category.keypointEdges],
        colors: [...this.category.keypointColors]
      },
  ...
  methods: {
    ...
    createAnnotation() {
      ...
      const annotationId = this.category.annotations.length
      Annotations.create({
        ...
        const annotations = this.$refs.annotation
        if (annotations == null) return

        const annotation = annotations[annotationId - 1]
        if (annotation == null) {
    ...
    onUpdateClick() {
      this.category.keypointLabels = [...this.keypoint.labels]
      this.category.keypointEdges = [...this.keypoint.edges]
      this.category.keypointColors = [...this.keypoint.colors]
    ...
    export() {
      ...
      const categoryData = {
        ...
        annotations: [],
        supercategory: this.category.supercategory,
        keypointLabels: this.category.keypointLabels,
        keypointEdges: this.category.keypointEdges,
        keypointColors: this.category.keypointColors
      }
    ...
    removeKeypointEdge(edge) {
      const index = this.keypoint.edges.findIndex(e => {
      ...
      if (index !== -1) {
        const edge = this.keypoint.edges[index]
     ...
     */
    onKeypointClick(annotationindex, keypointindex) {
      const indices = {
     ...
     */
    getAnnotation(index) {
      const ref = this.$refs.annotation
  ...
  sockets: {
    annotation(data) {
      ...
      if (annotation.imageId !== this.$parent.image.id) return
      if (annotation.categoryId !== this.category.id) return

      const found = this.category.annotations.findIndex(
