...
export default {
  ...
  methods: {
    ...
    generateTitle() {
      let string = ' '
      if (this.keypoint) {
        const index = this.keypoint.keypoint.indexLabel
        const label = this.keypoint.keypoints.labels[index - 1]
        const visibility = this.keypoint.keypoint.visibility
        const visibilityDesc = this.keypoint.keypoint.getVisibilityDescription()
        const annotationId = this.keypoint.keypoints.annotationId
        const categoryName = this.keypoint.keypoints.categoryName
      ...
      if (this.hover.category && this.hover.annotation) {
        const id = this.hover.textId
        const category = this.hover.category.category.name
        string += 'ID: ' + id + ' \n'
        string += 'Category: ' + category + ' \n'
      }

      if (this.$store.getters['user/loginEnabled']) {
        const creator = this.hover.annotation.annotation.creator
        if (creator != null) {
    ...
    generateStringFromMetadata() {
      ...
      let string = ''
      const metadata = this.hover.annotation.$refs.metadata.metadataList
    ...
    hoverText() {
      ...
      const position = this.hover.position.add(this.hover.textShift, 0)
      ...
      if (
        ...
        const content = this.generateTitle() + this.generateStringFromMetadata()
    ...
    checkBbox(paperObject) {
      ...
      const category = this.$parent.getCategory(categoryId)
    ...
    onMouseDrag(event) {
      if (this.isBbox && this.moveObject) {
        const deltaX = this.initPoint.x - event.point.x
        const deltaY = this.initPoint.y - event.point.y
        const segments = this.moveObject.children[0].segments
        segments.forEach(segment => {
          const p = segment.point
          segment.point = new paper.Point(p.x - deltaX, p.y - deltaY)
        })
        this.initPoint = event.point
      }
      if (this.segment && this.edit.canMove) {
        ...
        const deltaX = this.initPoint.x - event.point.x
        const deltaY = this.initPoint.y - event.point.y
        const centerDelta = new paper.Point(deltaX, deltaY)
        const newCenter = this.$parent.paper.view.center.add(centerDelta)
        this.$parent.paper.view.setCenter(newCenter)
      }
    },
        ...
        if (this.hover.text != null) {
          ...
          this.hover.shift =
            (this.hover.text.bounds.bottomRight.x -
              this.hover.text.bounds.bottomLeft.x) /
            2
          const totalShift = this.hover.shift + this.hover.textShift
