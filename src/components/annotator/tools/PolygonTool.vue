...
export default {
  ...
  methods: {
    ...
    autoStrokeColor(point) {
      ...
      const raster = this.$parent.image.raster
      const color = raster.getAverageColor(this.color.circle)
      if (color) {
    ...
    autoComplete(minCompleteLength) {
      ...
      const last = this.polygon.path.lastSegment.point
      const first = this.polygon.path.firstSegment.point

      const completeDistance = this.polygon.completeDistance
