...
export default {
  ...
  data() {
    return {
      imageError: false,
      selectedCategories: [],
      defaultMetadata: this.dataset.defaultAnnotationMetadata,
  ...
  methods: {
    ...
    onCocoDownloadClick() {
      const process = 'Generating COCO for ' + this.dataset.name
    ...
    onSave() {
      ...
      axios
        .post('/api/dataset/' + this.dataset.id, {
          ...
          defaultAnnotationMetadata: this.$refs.defaultAnnotation.export()
        })
        .then(() => {
          this.$parent.updatePage()
        })
  ...
  computed: {
    ...
    imageUrl() {
      ...
      if (this.dataset.numberImages > 0) {
        return '/api/image/' + this.dataset.first_imageId + '?width=250'
      }
    ...
    listCategories() {
      ...
      this.dataset.categories.forEach(category => {
        const elements = this.categories.filter(
