...
export class Keypoints extends paper.Group {
  ...
  addKeypoint(keypoint) {
    ...
    const indexLabel = keypoint.indexLabel
  ...
  deleteKeypoint(keypoint) {
    ...
    const index = this._keypoints.findIndex(k => k === keypoint)
  ...
  moveKeypoint(point, keypoint) {
    const indexLabel = keypoint.indexLabel
    const edges = this._edges[indexLabel]

    if (edges) {
      edges.forEach(i => {
        const line = this.getLine([i, indexLabel])
        if (line) {
  ...
  exportJSON(labels, width, height) {
    ...
    this._keypoints.forEach(k => {
      ...
      const point = k.clone().add(center)
  ...
  edges() {
    const edges = []
    const keys = Object.keys(this._edges)

    for (let i = 0; i < keys.length; i++) {
      const i1 = parseInt(keys[i])
      const otherIndices = Array.from(this._edges[i1])

      for (let j = 0; j < otherIndices.length; j++) {
        const i2 = parseInt(otherIndices[j])
  ...
  addEdge(edge) {
    ...
    const k1 = this._labelled[i1]
    const k2 = this._labelled[i2]
    if (k1 && k2) {
  ...
  _drawLines(keypoint) {
    ...
    const otherIndices = this._edges[keypoint.indexLabel]
    otherIndices.forEach(i => {
      const k2 = this._labelled[i]
      if (!k2) return

      const edge = [keypoint.indexLabel, i]
  ...
  _drawLine(edge, firstKeypoint, secondKeypoint) {
    const h = this._hashEdge(edge)
    if (this._lines[h]) return

    const line = new paper.Path.Line(firstKeypoint, secondKeypoint)
...
export const VisibilityOptions = (function() {
  const options = {}
  for (const l in VisibilityType) {
    if (l !== 'UNKNOWN') {
