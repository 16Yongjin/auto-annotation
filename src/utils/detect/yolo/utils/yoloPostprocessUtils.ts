...
export class ShapeUtil {
  ...
  static sizeFromDimension(dims: ReadonlyArray<number>, axis: number): number {
    if (axis > dims.length) {
      throw new Error(
        `invalid dimension of ${axis} for sizeFromDimension as Tensor has ${dims.length} dimensions.`
      )
    }
  ...
  static sizeToDimension(dims: ReadonlyArray<number>, axis: number): number {
    if (axis > dims.length) {
      throw new Error(
        `invalid dimension of ${axis} for sizeToDimension as Tensor has ${dims.length} dimensions.`
      )
    }
  ...
  static getSizeFromDimensionRange(
  ...
  static getActualAxisFromNegativeValue(
    axis: number,
    tensorRank: number
  ): number {
  ...
  static calculateReshapedDims(
    ...
    shapeHints: ReadonlyArray<number>
  ): number[] {
...
export class BroadcastUtil {
  ...
  static calcShape(
    ...
    if (isMatMul) {
      ...
      const cShapeMatMul = BroadcastUtil.calcMatMulShape(
    ...
    if (shape) {
      ...
      const c = ndarray(
        new (a.data.constructor as
          ...
          | Uint8ClampedArrayConstructor)(size),
        shape
      )
   ...
   */
  static isValidBroadcast(
    shape: ReadonlyArray<number>,
    finalShape: ReadonlyArray<number>
  ): boolean {
    ...
    for (let i = 1; i <= inputRank; i++) {
      if (
        shape[inputRank - i] !== 1 &&
        shape[inputRank - i] !== finalShape[finalRank - i]
      ) {
        return false
      }
    }
