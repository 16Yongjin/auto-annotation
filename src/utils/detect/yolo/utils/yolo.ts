import classNames from './yolo_classes'
import * as yolo from './yoloPostprocess'
...
export function yolo_head(feats: Tensor, anchors: Tensor, numClasses: number) {
  ...
  const convDimsTensor = yolo.cast(
    yolo.reshape(new Tensor(convDims, 'int32'), [1, 1, 1, 2]),
    feats.type
  )
...
export async function postprocess(outputTensor: Tensor, numClasses: number) {
  const [boxXy, boxWh, boxConfidence, boxClassProbs] = yolo_head(
    outputTensor,
    YOLO_ANCHORS,
    20
  )
