...
export function zeros(
  ...
  return new Tensor(
    TypedArrayUtil.createTypedArray(dtype, ShapeUtil.size(dims)),
    dtype,
    dims
  )
}
...
export function add(t1: Tensor, t2: Tensor): Tensor {
  if (
    (t1.type !== 'float32' && t1.type !== 'int32') ||
    (t2.type !== 'float32' && t2.type !== 'int32')
  ) {
    throw new Error('Unsupported type for transform')
  }
  if (t1.type !== t2.type) {
...
export function expandDims(t: Tensor, axis = 0): Tensor {
  axis = ShapeUtil.getActualAxisFromNegativeValue(
    axis,
...
export function slice(t: Tensor, begin: number[], size: number[]): Tensor {
  ...
  const oldDimsStride = ShapeUtil.computeStrides(
    t.dims ? t.dims : [t.data.length]
  )
  const X = t.data
...
export function greaterEqual(t1: Tensor, t2: Tensor): Tensor {
  if (
    (t1.type !== 'float32' && t1.type !== 'int32' && t1.type !== 'bool') ||
...
export function where(condition: Tensor, t1: Tensor, t2: Tensor): Tensor {
  ...
  ShapeUtil.areEqual(
    t1.dims ? t1.dims : [t1.data.length],
    t2.dims ? t2.dims : [t2.data.length]
  )
  ...
  const output = new Tensor(
    TypedArrayUtil.createTypedArray(t1.type, ShapeUtil.size(outputShape)),
    t1.type,
    outputShape
  )
  ...
  if (!condition.dims || condition.dims.length === 1) {
    ...
    const conditionDims = condition.dims
      ? condition.dims
      : [condition.data.length]
    const t1Dims = t1.dims ? t1.dims : [t1.data.length]
    if (conditionDims[0] !== t1Dims[0]) {
      throw new Error(
        'Outermost dimensions of input tensors and condition tensor must match'
      )
    }
    ...
    for (let i = 0; i < conditionData.length; ++i) {
      for (let j = 0; j < offset; ++j) {
        outputData[i * offset + j] =
          conditionData[i] > 0 ? X[i * offset + j] : Y[i * offset + j]
      }
    }
  } else {
...
export function cast(t: Tensor, dtype: Type): Tensor {
  ...
  switch (dtype) {
    case 'int32':
      return new Tensor(
        Int32Array.from(t.data as NumberDataType),
        'int32',
        t.dims ? t.dims : [t.data.length]
      )
    case 'float32':
      return new Tensor(
        Float32Array.from(t.data as NumberDataType),
        'float32',
        t.dims ? t.dims : [t.data.length]
      )
    case 'bool':
      return new Tensor(
        Uint8Array.from(t.data as NumberDataType),
        'bool',
        t.dims ? t.dims : [t.data.length]
      )
    default:
...
export function argMax(t: Tensor, axis = 0): Tensor {
  ...
  axis = ShapeUtil.getActualAxisFromNegativeValue(axis, rank)
  const [reduceDims, resultDims] = ShapeUtil.splitDimsIntoTwo(
    t.dims ? t.dims : [t.data.length],
    axis
  )
  const X = t.data
  const Y = TypedArrayUtil.createTypedArray(
    'int32',
    resultDims.length === 0 ? 1 : ShapeUtil.size(resultDims)
  )
...
export function max(t: Tensor, axis = 0, keepDims = false): Tensor {
  ...
  return new Tensor(
    Y,
    t.type,
    adjustedResultDims.length === 0 ? [1] : adjustedResultDims
  )
}
